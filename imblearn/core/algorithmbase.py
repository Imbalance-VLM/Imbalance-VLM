...
from imblearn.core.hooks import Hook, get_priority, CheckpointHook, TimerHook, LoggingHook, DistSamplerSeedHook, ParamUpdateHook, EvaluationHook, EMAHook, WANDBHook, AimHook
from imblearn.core.utils import get_dataset, get_data_loader, get_optimizer, get_cosine_schedule_with_warmup, Bn_Controller
from imblearn.core.criterions import CELoss, ConsistencyLoss
...
class AlgorithmBase:
    ...
    def get_freq(self,targets):
        ...
        self.lb_cls_num = self.lb_freq
        self.lb_freq = self.lb_freq / self.lb_freq.sum()
        print('labeled data frequency:', self.lb_freq)
        print('labeled data per cls num:', self.lb_cls_num)
    ...
    def set_dataset(self):
        ...
        dataset_dict = get_dataset(self.args, self.algorithm, self.args.dataset,  self.args.num_classes, self.args.data_dir)
        if dataset_dict is None:
            return dataset_dict

        self.args.lb_dest_len = len(dataset_dict['train_lb'])
        self.get_freq(dataset_dict['train_lb'].targets)
        self.print_fn("Total labeled data number {}".format(self.args.lb_dest_len))
        if self.rank == 0 and self.distributed:
            torch.distributed.barrier()
        return dataset_dict
